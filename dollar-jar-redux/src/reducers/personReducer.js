...
const personReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "ON-SUB-AMOUNT":
      let a = 0;
      let updatedState4 = [...state];
      let newPersonsArray3 = [...updatedState3.persons];
      for (a = 0; a < newPersonsArray3.length; a++) {
        if (newPersonsArray3[i].id === action.apyload) {
          break;
        }
      }

      let tempNew = { ...newPersonsArray3[a] };
      temp.total = temp.total + parseInt(updatedState3.jarValue);

      newPersonsArray3[a] = tempNew;

      return { ...state, persons: newPersonsArray3 };
